...
dir.create("Break In Output")
setwd("Break In Output")
...
for (mycsvfile in list.files(pattern = "csv", path = get("csvpath"))) {
  ...
  `Resting Em` <- df1 %>% 
    filter(between(`Time (ms)`, 0, 50) & Sweep == 9) %>% 
    summarise(mean_0_50 = mean(`Voltage (mV)`)) %>% 
    select(mean_0_50) %>% 
    unlist() %>% 
    as.numeric()
  
  `-50 pA step Steady-State Em` <- df1 %>% 
    filter(between(`Time (ms)`, 142, 158.7) & Sweep == 9) %>% 
    summarise(mean_147_158 = mean(`Voltage (mV)`)) %>% 
    select(mean_147_158) %>% 
    unlist() %>% 
    as.numeric()
  ...
  if (nrow(sweeps_AP_count) > 1) {
    ...
    Time1 <- (firstAPsweep_firstpeak %>% select(`Time (ms)`) %>% unlist() %>% as.numeric()) - 15
...
write_csv(all_data, "Computer Break In Data Analysis output.csv")

pdftools::pdf_combine(input = list.files(path = "Plots", full.names=TRUE, pattern=".pdf"),
                      output = "Break In All Plots.pdf")

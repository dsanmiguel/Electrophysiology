...
# can uncomment out which one you want
...
#intersect(incorrect_AP_counts$Filename_IV, correct_AP_counts$Filename_IV)
#nrow(correct_AP_counts)
#nrow(incorrect_AP_counts)
...
# this is to make sure that non_AP_count_errors doesn't include anything from incorrect_AP_counts
# should be 0 for output
# setdiff(non_AP_count_errors$Filename_IV, incorrect_AP_counts$Filename_IV) %>% length()

# this gets rid of the columns that have any zeroes in them
#new <- incorrect_AP_counts %>% select(where(~ any(. != 0)))
...
# we actually dont need this below at all! but let's leave it for shits and
